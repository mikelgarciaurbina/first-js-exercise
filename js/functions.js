...
function findLongestWord( array_words ){
	...
	for(var i=0; i < array_words.length; i++){
		if(array_words[i].length > max){
			max = array_words[i].length;
		}      
	}
...
array_lengths.push( findLongestWord( ["Basket", "Football"] ) );
...
function charFreq( string ) {
	...
	for( var i = 0; i < string.length; i++ ){
		if(!freqCounter[string[i]]) freqCounter[string[i]] = 0;
